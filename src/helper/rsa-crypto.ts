...
function _base64StringToArrayBuffer(b64str: any) {
...
function _arrayBufferToBase64(arrayBuffer: any) {
...
function convertPemToBinary(pem: any) {
 ...
 * @returns 加密後string
 */
export const encryptStringRsa = async (str: any, pemString: string) => {
  const textEncoder = new TextEncoder();
  const encodedPlaintext = textEncoder.encode(str).buffer;
  const encrypted = await encryptRsa(encodedPlaintext, pemString);
  const base64 = _arrayBufferToBase64(encrypted);
  return base64;
};
